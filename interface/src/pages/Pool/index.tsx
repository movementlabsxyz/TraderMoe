...
export default function Pool() {
  ...
  const showFarm = [SupportedChainId.APTOS].includes(chainId)
  // const showFarm = [SupportedChainId.APTOS, SupportedChainId.MOVEMENT_TESTNET].includes(chainId)
  ...
  const { moeCoin, nativeCoin, zUSDC } = getChainInfoOrDefault(chainId)
  ...
  useEffect(() => {
    const fetchStake = async () => {
      ...
      setAniPool({
        ...
        // @ts-ignore
        stakedLP: res.get(moeCoin?.address)?.amount,
        stakeAPR: res2[0]?.apr,
        // @ts-ignore
        earnedANI: res.get(moeCoin?.address)?.pendingAni,
      })
      setAptAniPool({
            ...
            <AutoColumn gap="md">
              ...
              <RowBetween>
                <ThemedText.DeprecatedWhite fontSize={14}>
                  <Trans>
                    Liquidity providers earn a 0.30% fee on all trades proportional to their share of the pool. Fees are
