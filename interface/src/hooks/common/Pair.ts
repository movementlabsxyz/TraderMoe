...
export function useAniPrice() {
  // rely on Header/index ConnectionInstance.getPair(moeCoin.address, stableCoin.address)
  const chainId = useChainId()
  const nativePrice = useNativePrice()
  const { nativeCoin, moeCoin } = getChainInfoOrDefault(chainId)
  const pair = usePair(nativeCoin.address, moeCoin.address)
  if (pair[0] === PairState.EXISTS) {
